...
我在构建项目时所用数据集来自老师发的文件：[东南大学云盘](https://pan.seu.edu.cn:443/link/215E44A851DA77CA52FF410F26F15498)， 下载需要校园网。

本仓库内不含数据文件，需要单独下载或将数据集自行处理后放入指定位置。

想要本地部署该项目，从以下两个渠道获取Flickr30k数据集：
 - **官方渠道：**[下载前两项即可](http://shannon.cs.illinois.edu/DenotationGraph/data/index.html)；
 - **国内网盘：**[百度网盘](https://pan.baidu.com/s/1r0RVUwctJsI0iNuVXHQ6kA)提取码hrf3。

### 数据集解包
 - 获取到的Flickr30k数据集，放入项目根目录`/Flickr30k`目录里；
 - 应该有两个文件：`flickr30k.tar.gz`和`flickr30k-images.tar`；
 - 准备好后，运行`prepare_flickr30k.py`，数据集会被划分至`/data`。

### 数据集介绍

训练集、验证集和测试集分别位于根目录下的`/train`、`/val`、`/test`文件夹。
